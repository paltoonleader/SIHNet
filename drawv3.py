...
def plot_metrics(all_labels, all_outputs, class_labels):
    ...
    sns.heatmap(conf_matrix, annot=True, fmt='d', cmap='Blues', cbar=False, ax=axs[0], 
                xticklabels=['SIH', 'Non-SIH'], yticklabels=['SIH', 'Non-SIH'], annot_kws={"size": 20})
    ...
    # Plot PR curves
    for i in range(len(class_labels)):
        axs[1].step(recall[i], precision[i], where='post', label=f'{class_labels[i]} (AUPRC={pr_auc[i]:.2f})')
    ...
    # Save the confusion matrix separately
    fig_conf_matrix, ax_conf_matrix = plt.subplots(figsize=(8, 6))
    sns.heatmap(conf_matrix, annot=True, fmt='d', cmap='Blues', cbar=False, ax=ax_conf_matrix, 
                xticklabels=['SIH', 'Non-SIH'], yticklabels=['SIH', 'Non-SIH'], annot_kws={"size": 20})
    ax_conf_matrix.set_title('Confusion Matrix')
    ax_conf_matrix.set_xlabel('Predicted Labels')
    ax_conf_matrix.set_ylabel('True Labels')
    plt.tight_layout()
    plt.savefig('confusion_matrix.jpg', format='jpg', bbox_inches='tight', pad_inches=0)
